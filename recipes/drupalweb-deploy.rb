...
if app['app_source']['type'] == 'git'
	...
	# TODO: Make this generic for any repo
	#
	bash "Add GHE to known_hosts" do
		code <<-EOS
		ssh-keyscan -f /var/www/.ssh/known_hosts -H git.links.com.au >> /var/www/.ssh/known_hosts
		EOS
		user 'apache'
		group 'apache'
...
if "#{app['app_source']['type']}" == 'archive' 
	...
	unless (::File.exists?"/var/www/sites/#{node['datashades']['sitename']}/robots.txt")
		...
		# Create Druapl core web directory
		#
...
# Link drupal default/files to shared_content folder on GFS
#
