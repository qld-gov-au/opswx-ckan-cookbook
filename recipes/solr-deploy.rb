...
unless system("grep '^#{account_name}:x:2001' /etc/passwd > /dev/null")
...
solr_version = solr_source[%r{/solr-([^/]+)[.]zip$}, 1]
solr_path = '/opt/solr'
...
unless ::File.identical?(installed_solr_version, solr_path)
  ...
  working_dir = if extra_disk_present
                  "#{extra_disk}/solr_install"
                else
                  '/tmp/solr_install'
                end
  ...
  unless node['datashades']['solr_password'].empty?
...
['1.2-api', 'api', 'core', 'slf4j-impl'].each do |jar_type|
