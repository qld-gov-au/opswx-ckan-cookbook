...
extordering =
  {
    ...
    'qgovext' => 25,
    'auth' => 27,  # ensure this is late enough for theme(s) to override
    'report' => 30,
    'datarequests' => 31,
    'ytp_comments' => 35,
    'csrf_filter' => 40,
...
node['datashades']['ckan_web']['plugin_app_names'].each do |plugin|
  if dependent_plugins.include? plugin
...
sorted_plugin_names.each do |plugin|
  ...
  5.times do
    ...
    next if plugin_parameters.nil? || (plugin_parameters == '')
    JSON.parse(plugin_parameters).each do |parameter|
      if parameter['Name'].end_with?('/shortname')
        egg_name = parameter['Value']
      elsif parameter['Name'].end_with?('/app_source/type')
        source_type = parameter['Value']
      elsif parameter['Name'].end_with?('/app_source/revision')
        source_revision = parameter['Value']
      elsif parameter['Name'].end_with?('/app_source/url')
  ...
  if pluginname.eql? 'archiver'
    ...
    if batchnode
      ...
      cron_d 'ckan-archiverTriggerAll' do
        minute 30
        hour 6
        day '1,15'
        user 'ckan'
        command '/usr/local/bin/pick-job-server.sh && /usr/local/bin/archiverTriggerAll.sh  >> /var/log/ckan/ckan-archiverTriggerAll.log 2>&1'
      end
    end
  end
  ...
  #
  next unless pluginname.eql? 'cesiumpreview'
...
unless resource_visibility_present
  cron_d 'Clean Resource Visibility cron' do
    cron_name 'ckan-dataset-resource-visibility-notify-privacy-assessments'
    action :delete
  end
end

unless report_present
  execute 'Clean Report cron' do
    command 'rm -f /etc/cron.daily/refresh_reports*'
  end
end

unless harvest_present
  if system('yum info supervisor')
